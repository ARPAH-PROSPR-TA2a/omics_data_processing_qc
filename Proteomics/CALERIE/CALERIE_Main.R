...
TIME_COL <- "SubjectID"        # Column name containing timepoint info
BASELINE_VALUES <- "CALERIE Baseline" # Values representing baseline

# PCA coloring variables
PCA_COLOR_VARS <- c("PlateId", "SlideId", "PlatePosition", "Subarray", "Sex", "Age")
...
norm_result <- somascan_norm_qc(dat,
                                lower = 0.4,
                                upper = 2.5,
                                mask_sample_ids = MASK_IDS)
...
write.csv(data.frame(
  ...
  percentages = c(norm_result$fail_counts$NormScale_20 / norm_result$pass_counts$NormScale_20 *100,
                  norm_result$fail_counts$NormScale_0_5 / norm_result$pass_counts$NormScale_0_5 *100, 
                  norm_result$fail_counts$NormScale_0_005 /  norm_result$pass_counts$NormScale_0_005 * 100,
                  norm_result$fail_counts$any_dilution/norm_result$pass_counts$all_dilutions * 100
                  )
